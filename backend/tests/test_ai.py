...
class TestAIEndpoints(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.customer_user = User(email="cust@ex.com", hashed_password="pw", role=UserRole.customer, is_active=True)
        ...
        self.worker_user = User(email="w1@ex.com", hashed_password="pw", role=UserRole.worker, is_active=True)
        ...
        self.customer_token = create_access_token(data={"sub": str(self.customer_user.id)})
        self.headers = {"Authorization": f"Bearer {self.customer_token}"}

    def tearDown(self) -> None:
        self.db.close()

    def test_search_services_fallback(self) -> None:
    ...
    def test_match_workers_fallback(self) -> None:
        ...
        self.assertEqual(response.json()["matches"][0]["worker"]["id"], self.worker.id)
        self.assertEqual(response.json()["matches"][0]["match_score"], 60.0)
        self.assertIn("Highly rated worker", response.json()["matches"][0]["match_reason"])
